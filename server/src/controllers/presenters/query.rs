...
impl Query {
    ...
    async fn find_order_by_id(&self, ctx: &Context<'_>, id: Uuid) -> Result<Order, Error> {
        let repo = ctx.data::<RepositoryProvider>().unwrap();
    ...
    async fn find_product_by_id(&self, ctx: &Context<'_>, id: i32) -> Result<Product, Error> {
        let repo = ctx.data::<RepositoryProvider>().unwrap();
    ...
    async fn find_user_by_email(&self, ctx: &Context<'_>, email: String) -> User {
        let repo = ctx.data::<RepositoryProvider>().unwrap();
